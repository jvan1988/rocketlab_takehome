...
Usage
    ...
    Send commands to the Simulator from another application or script over the network. The Simulator expects commands in a specific format. Here are some example commands:
        ...
        Discovery command: Send a command with the format ID; to discover the device.

        Start test command: Send a command with the format CMD=START;DURATION=10;RATE=100; to start a test. Adjust the DURATION and RATE values as needed.

        Stop test command: Send a command with the format CMD=STOP; to stop a running test.
